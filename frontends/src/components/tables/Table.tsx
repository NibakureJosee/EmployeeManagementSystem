...
export function CustomTable({
    ...
    const rows =
        sortedData.length === 0
            ? []
            : sortedData?.slice((page - 1) * pageSize, (page - 1) * pageSize + pageSize).map((item: any, item_index: number) => {
                return (
                    ...
                    {rows.length ? (
                        ...
                        <Table.Tr>
                            <Table.Td colSpan={columns.length + 1}>
                                <EmptyView message={errorFetching ? 'Error Fetching Data' : 'No Data To Show'} />
                            </Table.Td>
                        </Table.Tr>
                    ) : null}
                </Table.Tbody>
            </Table>
            <Pagination total={Math.ceil(sortedData.length / pageSize)} value={page} onChange={setPage} color="#0b7ff3" />
        </div>
    );
}